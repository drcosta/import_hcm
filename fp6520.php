...
$result_ori = $db->query("SELECT * FROM dgs01 WHERE acss = '00320226' AND (stat = '' OR stat = ' ' OR stat = 'L' OR stat = 'F' OR stat = 'I' OR stat = 'S' OR stat = 'M' OR stat = 'E') AND caus = '0' AND dqit = '00000000' AND ccst NOT LIKE '005%' AND ccst NOT LIKE '000%' ORDER BY acss ASC");
...
while ($row_ori = pg_fetch_object($result_ori)) {
  ...
  for ($i = 1; $i <= 12; $i++) {
    ...
    $result = $db->query("SELECT * FROM $tabela WHERE cdnn = '165' AND acss = '$acss_ori' ORDER BY acss ASC");
    ...
    while ($row = pg_fetch_object($result)) {
      ...
      if ($row->valo > 0 && $emp_estab["empresa"] > 0) {
        ...
        echo $row->valo . ";";     // Adicionais em valor para 13. salario
        echo ";";                                        // Adicionais em horas para férias
        echo $row->valo . ";";     // Adicionais em valor para férias
        echo ";;;;";
        echo "<br />";
//        echo "\n";
        $num++;
