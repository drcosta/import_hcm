...
$num = 1;
$ler = file('./de-para_mat.csv');
foreach ($ler as $linha) {
  $explode = explode(';', $linha);
  list($acss, $acss_hcm, $banco) = $explode;

  $acss = trim($acss);
  $banco = trim($banco);
  $acss_hcm = trim($acss_hcm);

  $db = new connection($banco);

  $result = $db->query("SELECT * FROM dgs01 WHERE acss = '$acss'");
...
//    echo "\n";
  ...
  //Tipo 3
    ...
    if ($msal + $csal > 0) {
      $sal_autal = str_replace('.', '', number_format($msal + $csal, '4'));
    } else if ($vsal > 0) {
      $sal_autal = str_replace('.', '', number_format($vsal, '4'));
    } else {
      $sal_autal = '26000';
    }
...
//  $sal_admin = str_replace('.', '', $ms + $cs) . "00";
    ...
    while ($row2 = pg_fetch_object($result2)) {
      if ($row2->msal > 0) {
        $ms = $row2->msal;
        $cs = $row2->csal;
        $sal_admin = number_format($ms + $cs, '4', '', '');
        break;
      }
    }
